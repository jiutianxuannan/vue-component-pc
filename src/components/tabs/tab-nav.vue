  ...
  export default {
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['y-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="y-icon-arrow-left"></i></span>,
          <span class={['y-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="y-icon-arrow-right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="y-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['y-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
          {scrollBtn}
          <div class={['y-tabs__nav-scroll']} ref="navScroll">
            <div
              class={['y-tabs__nav', `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
              ref="nav"
              style={navStyle}
              role="tablist"
...
<style scoped>
  ...
  .y-tabs__item:focus,
  .y-tabs__item:focus:active {
    outline: none;
  }

  .y-tabs__item:focus.is-active.is-focus:not(:active) {
    -webkit-box-shadow: 0 0 2px 2px #409EFF inset;
    box-shadow: 0 0 2px 2px #409EFF inset;
    border-radius: 3px;
  }

  .y-tabs__item .y-icon-close {
    border-radius: 50%;
    text-align: center;
  ...
  .y-tabs__item .y-icon-close:before {
    -webkit-transform: scale(0.9);
    transform: scale(0.9);
    display: inline-block;
  }

  .y-tabs__item .y-icon-close:hover {
    background-color: #C0C4CC;
    color: #FFFFFF;
  }

  .y-tabs__item.is-active {
    color: #409EFF;
  }
